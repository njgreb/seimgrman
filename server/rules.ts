...
export interface GameEvent {
  type: EventType;
  session: string | null; // one id per page load, generated by the game
...
// A UUID from crypto.randomUUID, or the plain-hex fallback in src/notify.ts.
const SESSION_PATTERN = /^[0-9a-f-]{8,36}$/i;
const isSession = (n: unknown): n is string => typeof n === 'string' && SESSION_PATTERN.test(n);
...
export function validateEvent(body: unknown): GameEvent | string {
  ...
  const event: GameEvent = {
    type,
    session: isSession(raw.session) ? raw.session : null,
    stats: null,
